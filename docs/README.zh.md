...
每个 profile 表示一个 llm。需要至少添加至少一个 profile
...
- 例 1：要求 llim 参考一个写作规范来完成写作任务，但是 llim 没有阅读写作规范文件，就开始输出文章，这种情况就是 llim 偷懒了，对指令遵循程度有问题。
- 例 2：给予 llim 一个有少许错误的指令，但可以通过 tool calling 获得正确的上下文。如果 llim 在已经获得正确上下文的前提下，依然指向了错误的指令，那么说明它缺少从错误中恢复的能力。
- 例 3：给予 llim 一个模糊的指令，但可以通过 tool calling 获得完整的上下文。如果 llim 在模糊指令中徘徊，执行了很多很多次 tool calling 才能理解（或者依然不能理解），说明 llim 的基础能力有问题。
...
- 内置 case 集非常小，只作为例子；实际测试需要自己编写自己需要的测试集。官方测试集细节不公开，但是数据可以在 [LLMSnare Arena](https://mistermorph.com/llmsnare/arena/) 查看 
...
内置 case 提供了最简单的用例。`llmsnare init` 会把这个内置 case 复制到磁盘 `~/.config/llmsnare/benchmarks/~/.config/llmsnare/benchmarks/` ，可以基于这个 sample 自由修改。
